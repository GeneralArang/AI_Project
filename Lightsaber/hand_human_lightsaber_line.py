...
def main():
    ...
    prev_t = 0.0
    base_line = None  # 처음 한 번만 잡는 기준선 (cx, cy, x, y)
    ...
    while cap.isOpened():
        ...
        if right_hand_kpts and len(right_hand_kpts) > 5:
            ...
            if base_line is None:
                base_line = (cx, cy, x, y)
                print(f"📍 기준선 고정: center=({cx},{cy}), hand=({x},{y})")

            # ===== 기준선 유지 =====
            if base_line is not None:
                bx1, by1, bx2, by2 = base_line
                cv2.line(frame, (bx1, by1), (bx2, by2), (0, 255, 255), 2)
                cv2.circle(frame, (bx1, by1), 6, (0, 255, 0), -1)
                cv2.circle(frame, (bx2, by2), 6, (0, 0, 255), -1)
